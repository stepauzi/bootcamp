...
public class StreamExercise {
 ...
 public static class Staff{
    ...
    public Staff(String name, boolean isMale){
      this.name = name;
      this.isMale = true;
    }
  ...
  public static void main(String[] args) {
    ...
    List<Integer> q1Result = numbers.stream()
      .filter(e -> e%2 == 0)
      .map( e -> e * e)
      .sorted((n1, n2) -> n1 > n2 ?-1 :1) //Comparator.reverseOrder()
    ...
    List<String> q2Result = names.stream()
    ...
    // Output: [5, 6, 4] (List)
    List<Integer> q4Result = words.stream()
    .map( e -> e.length())
    .collect(Collectors.toList());
    ...
    // Task: Given a list of Employee objects, filter employees with a salary greater than 50,000 and
    ...
    // Output: [John, Jane]
    List<String> q8Result = employeeList.stream()
    .filter(e -> e.getSalary() > 50000)
    .map(e -> e.getName()) //Employee::getName
    .collect(Collectors.toList());
    System.out.println(q8Result);
    ...
    List<Staff> staffList = Arrays.asList(
      new Staff("Alice", false),
      new Staff("Bob", true),
      new Staff("Charlie", true)
    );
    ...
    // Output: [30, 40, 60]
    List<Integer> numbers4List = numbers4.stream()
    .filter(n -> n > 10)
    .map(n -> n * 2)
    .collect(Collectors.toList());
    System.out.println(numbers4List);
